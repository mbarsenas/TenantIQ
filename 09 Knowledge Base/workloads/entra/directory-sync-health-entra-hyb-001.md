...
TenantIQ uses **ENTRA-HYB-001** to assess **directory sync health** in Entra ID. This guidance is intentionally evidence-bound: the assistant should explain only what the uploaded TenantIQ assessment actually observed and should not infer tenant configuration that is not present in the finding evidence.

## What this check represents

This check evaluates the TenantIQ control represented by: directory sync health, entra connect sync health, directory synchronization health.
