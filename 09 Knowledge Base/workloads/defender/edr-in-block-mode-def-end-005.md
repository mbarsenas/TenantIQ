...
TenantIQ uses **DEF-END-005** to assess **edr in block mode** in Microsoft Defender. This guidance is intentionally evidence-bound: the assistant should explain only what the uploaded TenantIQ assessment actually observed and should not infer tenant configuration that is not present in the finding evidence.

## What this check represents

This check evaluates the TenantIQ control represented by: edr in block mode, endpoint detection response block mode.
