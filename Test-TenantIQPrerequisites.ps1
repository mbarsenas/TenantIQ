...
Add-TenantIQPrecheckResult \
    -Item 'PowerShell' \
    -Status $(if ($PowerShellOk) { 'OK' } else { 'MISSING' }) \
    -Detected $PowerShellVersion \
    -Needed '7.0 or later' \
    -Fix $(if ($PowerShellOk) { '' } else { 'Install PowerShell 7 and run this script from pwsh.exe.' })
...
Add-TenantIQPrecheckResult \
    ...
    -Status $(if ($ExecutionPolicyOk) { 'OK' } else { 'REVIEW' }) \
    -Detected ([string]$ExecutionPolicy) \
    -Needed 'Allows local TenantIQ scripts to run' \
    -Fix $(if ($ExecutionPolicyOk) { '' } else { 'Use pwsh -ExecutionPolicy Bypass for troubleshooting, or review your organization policy.' })

$Gallery = Get-PSRepository -Name PSGallery
$GalleryOk = $null -ne $Gallery
Add-TenantIQPrecheckResult \
    -Item 'PowerShell Gallery' \
    -Status $(if ($GalleryOk) { 'OK' } else { 'MISSING' }) \
    -Detected $(if ($GalleryOk) { "Registered ($($Gallery.InstallationPolicy))" } else { 'Not registered' }) \
    -Needed 'PSGallery available for module installation' \
    -Fix $(if ($GalleryOk) { '' } else { 'Run Register-PSRepository -Default from an elevated PowerShell session if your organization permits it.' })

$NuGet = Get-PackageProvider -Name NuGet | Sort-Object Version -Descending | Select-Object -First 1
$NuGetOk = $null -ne $NuGet
Add-TenantIQPrecheckResult \
    -Item 'NuGet Provider' \
    -Status $(if ($NuGetOk) { 'OK' } else { 'MISSING' }) \
    -Detected $(if ($NuGetOk) { [string]$NuGet.Version } else { 'Not installed' }) \
    -Needed 'NuGet package provider' \
    -Fix $(if ($NuGetOk) { '' } else { 'Run Install-PackageProvider -Name NuGet -Scope CurrentUser -Force.' })
...
foreach ($ModuleName in $RequiredModules) {
    ...
    Add-TenantIQPrecheckResult \
        ...
        -Detected $(if ($Installed) { [string]$Module.Version } else { 'Not installed' }) \
        -Needed 'Required' \
        -Fix $(if ($Installed) { '' } else { "Install-Module -Name $ModuleName -Scope CurrentUser -Force -AllowClobber" })
}
...
if ($IncludeOptional) {
    foreach ($ModuleName in $OptionalModules) {
        ...
        Add-TenantIQPrecheckResult \
            -Item $ModuleName \
            -Status $(if ($Installed) { 'OK' } else { 'OPTIONAL' }) \
            -Detected $(if ($Installed) { [string]$Module.Version } else { 'Not installed' }) \
            -Needed 'Optional' \
            -Fix $(if ($Installed) { '' } else { "Install-Module -Name $ModuleName -Scope CurrentUser -Force -AllowClobber" })
    }
...
Add-TenantIQPrecheckResult \
    -Item 'TenantIQ Launcher' \
    -Status $(if ($LauncherOk) { 'OK' } else { 'MISSING' }) \
    -Detected $(if ($LauncherOk) { $LauncherPath } else { 'Start-TenantIQ.ps1 not found' }) \
    -Needed 'Required' \
    -Fix $(if ($LauncherOk) { '' } else { 'Run the pre-check from the TenantIQ package root.' })
