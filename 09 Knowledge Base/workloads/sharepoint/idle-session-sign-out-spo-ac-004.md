...
TenantIQ uses **SPO-AC-004** to assess **idle session sign out** in SharePoint Online. This guidance is intentionally evidence-bound: the assistant should explain only what the uploaded TenantIQ assessment actually observed and should not infer tenant configuration that is not present in the finding evidence.

## What this check represents

This check evaluates the TenantIQ control represented by: idle session sign out, idle session sign-out.
