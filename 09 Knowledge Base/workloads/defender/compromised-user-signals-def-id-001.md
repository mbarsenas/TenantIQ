...
# Compromised User Signals

TenantIQ uses **DEF-ID-001** to assess **compromised user signals** in Microsoft Defender. This guidance is intentionally evidence-bound: the assistant should explain only what the uploaded TenantIQ assessment actually observed and should not infer tenant configuration that is not present in the finding evidence.

## What this check represents

This check evaluates the TenantIQ control represented by: compromised user signals, identity compromised user signals.
