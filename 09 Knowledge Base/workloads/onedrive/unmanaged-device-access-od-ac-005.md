...
TenantIQ uses **OD-AC-005** to assess **unmanaged device access** in OneDrive. This guidance is intentionally evidence-bound: the assistant should explain only what the uploaded TenantIQ assessment actually observed and should not infer tenant configuration that is not present in the finding evidence.

## What this check represents

This check evaluates the TenantIQ control represented by: unmanaged device access, onedrive unmanaged device access.
