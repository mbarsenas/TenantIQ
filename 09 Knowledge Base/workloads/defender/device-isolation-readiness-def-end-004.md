...
# Device Isolation Readiness

TenantIQ uses **DEF-END-004** to assess **device isolation readiness** in Microsoft Defender. This guidance is intentionally evidence-bound: the assistant should explain only what the uploaded TenantIQ assessment actually observed and should not infer tenant configuration that is not present in the finding evidence.

## What this check represents

This check evaluates the TenantIQ control represented by: device isolation readiness, endpoint device isolation.
