...
if($catalog -and (Get-Command Connect-PnPOnline -ErrorAction SilentlyContinue)){
    $PnPConnected=$false
    try{
        Connect-PnPOnline -Url $catalog.Url -Interactive -ErrorAction Stop | Out-Null
        $PnPConnected=$true
        $apps=@(Get-PnPApp -Scope Tenant -ErrorAction Stop)
        $coverage='PnP tenant app catalog package inventory'
    }catch{
        $coverage="PnP enumeration failed: $($_.Exception.Message)"
    }finally{
