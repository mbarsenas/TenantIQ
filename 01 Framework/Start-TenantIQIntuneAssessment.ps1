function Start-TenantIQIntuneAssessment {
    Clear-Host
    $Global:ExchangeAIResults = @()

    $Checks = @($TenantIQIntuneHealthChecks | Where-Object { $_.Enabled -ne $false })
    $Total = $Checks.Count
    $i = 1
    ...
    Write-Host '              TenantIQ Microsoft Intune Assessment' -ForegroundColor Cyan
    Write-Host '============================================================' -ForegroundColor Cyan
    Write-Host ''
    ...
    foreach ($Check in $Checks) {
        ...
        try {
            ...
            Add-TenantIQBulkResult `
                ...
                -Recommendation 'Review Microsoft Intune/Graph connectivity, permissions, licensing, or check dependencies.' `
                -Duration 0
        }
        ...
        if ($After -eq $Before) {
            Add-TenantIQBulkResult `
                ...
                -Status 'NOT EVALUATED' `
                -Severity 'None' `
                -Finding 'The Microsoft Intune check executed but did not return a standardized TenantIQ result.' `
                -Recommendation 'Validate this control and its Microsoft Graph dependency before using it as a scored production finding.' `
                -Duration 0
        }
        $i++
    }
    ...
    Write-Host ''
    Write-Host '============================================================' -ForegroundColor Cyan
    Write-Host '              Microsoft Intune Assessment Complete' -ForegroundColor Cyan
    Write-Host '============================================================' -ForegroundColor Cyan
    Write-Host ''
    ...
    if ($Results.Count -gt 0) {
        try {
            $Report = Export-ExchangeAIHtmlReport -Workload 'Microsoft Intune'
            if ($Report.HtmlPath) { Start-Process $Report.HtmlPath }
        }
        catch {
            Write-Host 'Unable to generate Microsoft Intune HTML report.' -ForegroundColor Red
