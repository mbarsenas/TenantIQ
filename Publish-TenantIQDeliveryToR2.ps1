...
if ([string]::IsNullOrWhiteSpace($AwsProfile)) {
    if ([string]::IsNullOrWhiteSpace($AccessKeyId)) { throw 'TENANTIQ_R2_ACCESS_KEY_ID is not set and -AwsProfile was not provided.' }
    if ([string]::IsNullOrWhiteSpace($SecretAccessKey)) { throw 'TENANTIQ_R2_SECRET_ACCESS_KEY is not set and -AwsProfile was not provided.' }
}
...
try {
    ...
    $uploadArgs = @(
        's3','cp',$PackagePath,"s3://$BucketName/$objectKey",
...
if ($AwsProfile) { Write-Host ('AWS Profile  : {0}' -f $AwsProfile) }
