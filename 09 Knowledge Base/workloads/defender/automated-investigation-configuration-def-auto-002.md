...
# Automated Investigation Configuration

TenantIQ uses **DEF-AUTO-002** to assess **automated investigation configuration** in Microsoft Defender. This guidance is intentionally evidence-bound: the assistant should explain only what the uploaded TenantIQ assessment actually observed and should not infer tenant configuration that is not present in the finding evidence.

## What this check represents

This check evaluates the TenantIQ control represented by: automated investigation configuration, air configuration.
