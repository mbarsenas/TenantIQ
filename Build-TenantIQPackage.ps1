...
$RequiredFiles = @(
    ...
    'Test-TenantIQPrerequisites.ps1',
...
$RequiredDirectories = @(
    '00 Runtime','01 Framework','02 Health Checks','03 Reports','04 Scripts','05 Templates','07 Assets','10 Modules'
)

$BuildCommit = $null
try { if (Get-Command git -ErrorAction SilentlyContinue) { $BuildCommit = (& git -C $Root rev-parse --short=12 HEAD 2>$null).Trim() } } catch {}
...
foreach ($File in $RequiredFiles) {
    if (-not (Test-Path (Join-Path $Root $File))) {
        if ($File -eq 'TenantIQ-License-Public.pem') { throw 'TenantIQ signing public key is not configured. Run .\Initialize-TenantIQLicenseKeys.ps1 once, then commit only TenantIQ-License-Public.pem.' }
        throw "Required package file is missing: $File"
    }
}
foreach ($Directory in $RequiredDirectories) { if (-not (Test-Path (Join-Path $Root $Directory))) { throw "Required package directory is missing: $Directory" } }

if (Test-Path $PackageRoot) { Remove-Item -Path $PackageRoot -Recurse -Force }
if (Test-Path $ZipPath) { Remove-Item -Path $ZipPath -Force }
if (Test-Path "$ZipPath.sha256") { Remove-Item -Path "$ZipPath.sha256" -Force }
New-Item -Path $PackageRoot -ItemType Directory -Force | Out-Null

foreach ($File in $RequiredFiles) { Copy-Item -Path (Join-Path $Root $File) -Destination (Join-Path $PackageRoot $File) -Force }
foreach ($Directory in $RequiredDirectories) { Copy-Item -Path (Join-Path $Root $Directory) -Destination (Join-Path $PackageRoot $Directory) -Recurse -Force }
...
$PatternsToRemove = @('*.analysis.json','*.log','*.tmp','*.bak','*.old','*.orig','*~')
foreach ($Pattern in $PatternsToRemove) { Get-ChildItem -Path $PackageRoot -Recurse -File -Filter $Pattern -ErrorAction SilentlyContinue | Remove-Item -Force -ErrorAction SilentlyContinue }

$PackagedLicense = Join-Path $PackageRoot 'TenantIQ-License.json'
if (Test-Path $PackagedLicense) { Remove-Item -Path $PackagedLicense -Force }
Get-ChildItem -Path $PackageRoot -Recurse -Force -Directory -Filter '.git' -ErrorAction SilentlyContinue | Remove-Item -Recurse -Force -ErrorAction SilentlyContinue
...
$ManifestRows = Get-ChildItem -Path $PackageRoot -Recurse -File | Where-Object { $_.FullName -ne $ManifestPath } | Sort-Object FullName | ForEach-Object {
...
Write-Host '  .\Test-TenantIQPrerequisites.ps1'
Write-Host '  .\Test-TenantIQTenantAccess.ps1'
