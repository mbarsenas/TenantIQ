...
TenantIQ uses **INTUNE-WIN-002** to assess **autopilot esp configuration** in Microsoft Intune. This guidance is intentionally evidence-bound: the assistant should explain only what the uploaded TenantIQ assessment actually observed and should not infer tenant configuration that is not present in the finding evidence.

## What this check represents

This check evaluates the TenantIQ control represented by: autopilot esp configuration, enrollment status page configuration.
