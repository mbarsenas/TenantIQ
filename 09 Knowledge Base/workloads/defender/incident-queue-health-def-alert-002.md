...
# Incident Queue Health

TenantIQ uses **DEF-ALERT-002** to assess **incident queue health** in Microsoft Defender. This guidance is intentionally evidence-bound: the assistant should explain only what the uploaded TenantIQ assessment actually observed and should not infer tenant configuration that is not present in the finding evidence.

## What this check represents

This check evaluates the TenantIQ control represented by: incident queue health, defender incident queue health.
