function Start-TenantIQSharePointAssessment {
    ...
    $Checks = @($TenantIQSharePointHealthChecks | Where-Object { $_.Enabled -ne $false })
    $Total = $Checks.Count
    $i = 1
    ...
    foreach ($Check in $Checks) {
        ...
        try {
            ...
            Add-TenantIQBulkResult -Check $Check.Name -Category $Check.Category -Status 'NOT EVALUATED' -Severity 'None' -Finding $_.Exception.Message -Recommendation 'Review SharePoint Online connectivity, permissions, licensing, or check dependencies.' -Duration 0
        }
        $After = @($Global:ExchangeAIResults).Count
        if ($After -eq $Before) {
            Add-TenantIQBulkResult -Check $Check.Name -Category $Check.Category -Status 'NOT EVALUATED' -Severity 'None' -Finding 'The SharePoint Online check executed but did not return a standardized TenantIQ result.' -Recommendation 'Validate this control and its workload dependency before using it as a scored production finding.' -Duration 0
        }
        $i++
    }
    ...
    if ($Results.Count -gt 0) {
        try {
            $Report = Export-ExchangeAIHtmlReport -Workload 'SharePoint Online'
            if ($Report.HtmlPath) { Start-Process $Report.HtmlPath }
        }
        catch {
            Write-Host 'Unable to generate SharePoint Online HTML report.' -ForegroundColor Red
