...
# Exchange Dlp Coverage

TenantIQ uses **PUR-DLP-006** to assess **exchange dlp coverage** in Microsoft Purview. This guidance is intentionally evidence-bound: the assistant should explain only what the uploaded TenantIQ assessment actually observed and should not infer tenant configuration that is not present in the finding evidence.

## What this check represents

This check evaluates the TenantIQ control represented by: exchange dlp coverage, exchange data loss prevention coverage.
