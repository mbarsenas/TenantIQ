...
TenantIQ uses **SPO-SHR-003** to assess **external sharing** in SharePoint Online. This guidance is intentionally evidence-bound: the assistant should explain only what the uploaded TenantIQ assessment actually observed and should not infer tenant configuration that is not present in the finding evidence.

## What this check represents

This check evaluates the TenantIQ control represented by: external sharing, sharepoint external sharing.
