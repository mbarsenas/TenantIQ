...
TenantIQ uses **DEF-ALERT-003** to assess **suppression rules** in Microsoft Defender. This guidance is intentionally evidence-bound: the assistant should explain only what the uploaded TenantIQ assessment actually observed and should not infer tenant configuration that is not present in the finding evidence.

## What this check represents

This check evaluates the TenantIQ control represented by: suppression rules, alert suppression rules.
