...
TenantIQ uses **ENTRA-APP-010** to assess **tenant app management policy** in Entra ID. This guidance is intentionally evidence-bound: the assistant should explain only what the uploaded TenantIQ assessment actually observed and should not infer tenant configuration that is not present in the finding evidence.

## What this check represents

This check evaluates the TenantIQ control represented by: tenant app management policy, tenant application management policy, app management policy.
