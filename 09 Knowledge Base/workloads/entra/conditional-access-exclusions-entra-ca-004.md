...
TenantIQ uses **ENTRA-CA-004** to assess **conditional access exclusions** in Entra ID. This guidance is intentionally evidence-bound: the assistant should explain only what the uploaded TenantIQ assessment actually observed and should not infer tenant configuration that is not present in the finding evidence.

## What this check represents

This check evaluates the TenantIQ control represented by: conditional access exclusions, ca exclusions, conditional access policy exclusions.
