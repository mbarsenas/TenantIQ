...
TenantIQ uses **EXO-MF-007** to assess **transport rules** in Exchange Online. This guidance is intentionally evidence-bound: the assistant should explain only what the uploaded TenantIQ assessment actually observed and should not infer tenant configuration that is not present in the finding evidence.

## What this check represents

This check evaluates the TenantIQ control represented by: transport rules, mail flow rules, exchange transport rules.
