...
TenantIQ uses **ENTRA-ID-004** to assess **domain configuration** in Entra ID. This guidance is intentionally evidence-bound: the assistant should explain only what the uploaded TenantIQ assessment actually observed and should not infer tenant configuration that is not present in the finding evidence.

## What this check represents

This check evaluates the TenantIQ control represented by: domain configuration, entra domain configuration.
