...
function Invoke-TenantIQAccessProbe {
    ...
    if (Get-CommandAvailable 'Get-ConnectionInformation') {
        try {
            ...
            if ($exo.Count -gt 0) {
                $exoConnected=$true
                $e=$exo | Select-Object -First 1
                $exoDetail=if($e.UserPrincipalName){"Connected as $($e.UserPrincipalName)"}else{'Connected session detected'}
            }
    ...
    if ($exoConnected) {
        $results.Add((New-AccessResult 'Exchange Online' 'OK' 'Access confirmed' $exoDetail))
    } else {
    ...
    if($spoConnected){
        $spoStatus='OK'
        $results.Add((New-AccessResult 'SharePoint Online' 'OK' 'Access confirmed' 'SharePoint Online tenant query succeeded.'))
    } else {
    ...
    if(Get-CommandAvailable 'Get-CsTenant'){
        try{$csTenant=Get-CsTenant -ErrorAction Stop;if($csTenant){$teamsConnected=$true}}catch{$teamsError=$_.Exception.Message}
    } else {$teamsError='Microsoft Teams cmdlets are not available in the current session.'}
    if($teamsConnected){
        $results.Add((New-AccessResult 'Microsoft Teams' 'OK' 'Access confirmed' 'Microsoft Teams tenant query succeeded.'))
    } else {
        ...
        $results.Add((New-AccessResult 'Microsoft Teams' $status $category $detail 'Run Connect-MicrosoftTeams and authenticate.'))
    }
    ...
    if($graphContext -and $graphContext.Account -and (Get-CommandAvailable 'Invoke-MgGraphRequest')){
        try{Invoke-MgGraphRequest -Method GET -Uri 'https://graph.microsoft.com/v1.0/deviceManagement/managedDevices?$top=1' -OutputType PSObject -ErrorAction Stop|Out-Null;$intuneOk=$true}catch{$intuneError=$_.Exception.Message}
    }
    ...
    if($graphContext -and $graphContext.Account -and (Get-CommandAvailable 'Invoke-MgGraphRequest')){
        try{Invoke-MgGraphRequest -Method GET -Uri 'https://graph.microsoft.com/v1.0/security/alerts_v2?$top=1' -OutputType PSObject -ErrorAction Stop|Out-Null;$defenderOk=$true}catch{$defenderError=$_.Exception.Message}
    }
...
function Show-TenantIQAccessResults {
    ...
    foreach($Result in $Results){
        $Color=switch($Result.Status){'OK'{'Green'}'SIGN-IN REQUIRED'{'Yellow'}'REVIEW'{'Yellow'}'DENIED'{'Red'}'UNAVAILABLE'{'Yellow'}'ERROR'{'Red'}'BLOCKED'{'Red'}default{'Gray'}}
    ...
    $signin=@($Results|Where-Object Status -eq 'SIGN-IN REQUIRED').Count
    $review=@($Results|Where-Object Status -eq 'REVIEW').Count
    $problems=@($Results|Where-Object{$_.Status -in @('DENIED','UNAVAILABLE','ERROR','BLOCKED')}).Count

    Write-Host ''
    Write-Host 'Summary' -ForegroundColor Cyan
    Write-Host '-------' -ForegroundColor Cyan
    Write-Host ('OK                : {0}' -f $ok) -ForegroundColor Green
    Write-Host ('Sign-in required  : {0}' -f $signin) -ForegroundColor Yellow
    Write-Host ('Review            : {0}' -f $review) -ForegroundColor Yellow
    Write-Host ('Problems          : {0}' -f $problems) -ForegroundColor $(if($problems -gt 0){'Red'}else{'Green'})
    Write-Host ''

    if($problems -gt 0){Write-Host '[NOT READY] One or more workload access problems need attention.' -ForegroundColor Red}
    elseif($signin -gt 0){Write-Host '[ACTION REQUIRED] Please sign in to the workloads listed above.' -ForegroundColor Yellow}
    elseif($review -gt 0){Write-Host '[REVIEW] Core access is available, but one or more workloads still need feature validation.' -ForegroundColor Yellow}
    else{Write-Host '[READY] All TenantIQ workload access checks passed.' -ForegroundColor Green}
...
function Invoke-TenantIQInteractiveSignIn {
    ...
    if ($needGraph) {
        ...
        try {
            $graphScopes = @(
                ...
                'Application.Read.All','Group.Read.All','User.Read.All','Reports.Read.All',
                'DeviceManagementManagedDevices.Read.All','SecurityAlert.Read.All'
            )
            Connect-MgGraph -Scopes $graphScopes -NoWelcome -ErrorAction Stop | Out-Null
            Write-Host '[OK] Microsoft Graph sign-in completed.' -ForegroundColor Green
        } catch { Write-Host ('[WARNING] Microsoft Graph sign-in did not complete: {0}' -f $_.Exception.Message) -ForegroundColor Yellow }
    }

    if ($needExchange) {
        Write-Host ''
        Write-Host 'Signing in to Exchange Online...' -ForegroundColor Cyan
        try { Connect-ExchangeOnline -ShowBanner:$false -ErrorAction Stop | Out-Null; Write-Host '[OK] Exchange Online sign-in completed.' -ForegroundColor Green }
        catch { Write-Host ('[WARNING] Exchange Online sign-in did not complete: {0}' -f $_.Exception.Message) -ForegroundColor Yellow }
    }
    ...
    if ($needSharePoint) {
        ...
        if ($tenantStem) {
            ...
            try { Connect-SPOService -Url $adminUrl -ErrorAction Stop; Write-Host '[OK] SharePoint Online sign-in completed.' -ForegroundColor Green }
            catch { Write-Host ('[WARNING] SharePoint Online sign-in did not complete: {0}' -f $_.Exception.Message) -ForegroundColor Yellow }
        } else {
            Write-Host '[WARNING] Could not determine the SharePoint tenant name. SharePoint sign-in skipped.' -ForegroundColor Yellow
        }
    }
    ...
    if ($needTeams) {
        ...
        try { Connect-MicrosoftTeams -ErrorAction Stop | Out-Null; Write-Host '[OK] Microsoft Teams sign-in completed.' -ForegroundColor Green }
        catch { Write-Host ('[WARNING] Microsoft Teams sign-in did not complete: {0}' -f $_.Exception.Message) -ForegroundColor Yellow }
    }
}
...
$results = Invoke-TenantIQAccessProbe
$summary = Show-TenantIQAccessResults -Results $results

if (-not $NoPrompt -and $summary.SignInRequired -gt 0) {
    Write-Host ''
    $answer = Read-Host ("{0} workload(s) require authentication. Would you like TenantIQ to start the required sign-ins now? [Y/N]" -f $summary.SignInRequired)
    if ($answer -match '^(?i)y(?:es)?$') {
        Invoke-TenantIQInteractiveSignIn -CurrentResults $results
        Write-Host ''
        Write-Host 'Re-running TenantIQ tenant access checks...' -ForegroundColor Cyan
        Write-Host ''
        $results = Invoke-TenantIQAccessProbe
        $summary = Show-TenantIQAccessResults -Results $results
    }
